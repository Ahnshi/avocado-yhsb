...
var prev_co_id = 0;
function re_comment_box(co_id, wr_id) { 
	var re_comment_obj = $('#bo_vc_w_'+wr_id).clone();
	console.log(re_comment_obj);
	re_comment_obj.attr('id', 'vo_vc_re_w_'+co_id);
	re_comment_obj.find('*[name="fviewcomment"]').attr('name', 're_fviewcomment'+co_id);
	re_comment_obj.find('*[name="comment_id"]').val(co_id);
	console.log($('#c_'+co_id));
	
	if($('#c_'+co_id).find('*#vo_vc_re_w_'+co_id).length == 0)
		$('#c_'+co_id).append(re_comment_obj);
	else
		$('#vo_vc_re_w_'+co_id).toggle();
