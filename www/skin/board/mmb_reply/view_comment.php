...
// 원문 출력하기
$sql = " select * from $write_table where wr_id = '{$list_item['wr_id']}' and wr_is_comment = 0";
...
$sql = " select * from $write_table where wr_parent = '{$list_item['wr_id']}' and wr_is_comment = 1 order by wr_comment, wr_comment_reply, wr_datetime asc ";
//$sql = " select * from $write_table where wr_parent = '{$list_item['wr_id']}' and wr_content != '' order by wr_datetime asc ";
$result = sql_query($sql);
for ($c_i=1; $c_row=sql_fetch_array($result); $c_i++)
{
	$comment[$c_i] = $c_row;
