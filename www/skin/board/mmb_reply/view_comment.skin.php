...
for ($index=0; $index<count($comment); $index++) {
	...
	if($list_item['wr_id'] == $log_comment['wr_id']) {
		$log_comment['is_reply'] = false;
		$log_comment['is_edit'] = false;
		$log_comment['is_del'] = false;
	}
	
	if($list_item['wr_id'] != $log_comment['wr_id'] && ($log_comment['is_reply'] || $log_comment['is_edit'] || $log_comment['is_del'])) {
		// 답변, 수정, 삭제가 가능할 경우
...
<div class="item-comment <?php if ($cmt_depth) { ?>reply-<?=$cmt_depth?><?php } ?>" id="c_<?php echo $comment_id ?>" style="margin-left:<?=$cmt_depth?>px;">
	<div class="co-header">
	...
	<div class="co-content">
		...
		<div id="reply_<?php echo $comment_id ?>"></div>
	</div>
	...
	<div class="co-footer">
		...
		<!--<?php if ($log_comment['is_reply']) { ?><a href="<?php echo $c_reply_href;  ?>" onclick="re_comment_box('<?php echo $comment_id ?>'); return false;" class="re">답변</a><?php } ?>-->
		<!-- 2z수정(22.1.15) 답변에 wr_id가 필요하여 wr_id를 받도록 추가 -->
		<?php if ($log_comment['is_reply']) { ?><a href="<?php echo $c_reply_href;  ?>" onclick="re_comment_box('<?php echo $comment_id ?>', '<?=$list_item['wr_id']?>'); return false;" class="re">답변</a><?php } ?>
	</div>
</div>
<? } ?>
